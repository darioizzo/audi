...
#define AUDI_MAX_OUTPUT_LENGTH 5u
...
inline void stream_impl(std::ostream &os, const std::vector<T> &v)
{
    ...
    if (len <= AUDI_MAX_OUTPUT_LENGTH) {
        ...
        os << '[';
        for (decltype(v.size()) i = 0u; i < AUDI_MAX_OUTPUT_LENGTH; ++i) {
            stream(os, v[i], ", ");
        }
        os << "... ]";
    }
...
inline void stream_impl(std::ostream &os, const std::map<T, U> &m)
{
    ...
    for (auto it = m.begin(); it != m.end(); ++counter) {
        if (counter == AUDI_MAX_OUTPUT_LENGTH) {
            stream(os, "...");
            break;
        }
...
// A small helper function that transforms x to string, using internally audi::stream.
template <typename T>
...
public:
    ...
    void add_row(const Args &... args)
    {
        if (sizeof...(args) != m_headers.size()) {
            audi_throw(std::invalid_argument, "the table was constructed with " + to_string(m_headers.size())
...
/// The audi print function.
/**
 * This function is equivalent to calling audi::stream with \p std::cout as first argument.
 *
