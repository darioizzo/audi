...
if is_python_build:
    ...
    run_command(r'cmake -G "MinGW Makefiles" ..  -DPYAUDI_INSTALL_PATH=c:\\local -DAUDI_BUILD_AUDI=no -DAUDI_BUILD_PYAUDI=yes -DCMAKE_BUILD_TYPE=Release ' + common_cmake_opts + ' ' +
                r'-DPYTHON_EXECUTABLE=C:\\Python' + python_version + r'\\python.exe -DPYTHON_LIBRARY=C:\\Python' + python_version + r'\\libs\\python' + python_version + r'.dll' + r' -DPYBIND11_EXPORT="__attribute__ ((visibility(\"default\")))"')
    run_command(r'mingw32-make install VERBOSE=1 -j2')
