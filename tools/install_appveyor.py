...
os.chdir('build')
print("Installing mp++")
run_command(r'cmake -G "MinGW Makefiles" .. -DCMAKE_INSTALL_PREFIX=c:\\local -DCMAKE_PREFIX_PATH=c:\\local -DMPPP_WITH_MPFR=yes -DCMAKE_CXX_STANDARD=17', verbose=True)
...
run_command(r'cmake -G "MinGW Makefiles" .. -DCMAKE_INSTALL_PREFIX=c:\\local -DCMAKE_PREFIX_PATH=c:\\local -DCMAKE_CXX_STANDARD=17 -DCMAKE_CXX_FLAGS=-fPIC', verbose=True)
run_command(r'mingw32-make install VERBOSE=1', verbose=True)
os.chdir('../../')
...
run_command(r'cmake -G "MinGW Makefiles" .. -DCMAKE_INSTALL_PREFIX=c:\\local -DCMAKE_PREFIX_PATH=c:\\local -DBoost_INCLUDE_DIR=c:\\local\\include -DBoost_SERIALIZATION_LIBRARY_RELEASE=c:\\local\\lib\\libboost_serialization-mgw81-mt-x64-1_70.dll', verbose=True)
run_command(r'mingw32-make install VERBOSE=1', verbose=True)
os.chdir('../../')
print("obake successfully installed .. continuing")
