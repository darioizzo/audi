...
if is_python_build:
    ...
    run_command(r'cmake -G "MinGW Makefiles" ..' + r' ' + 
                ...
                r'-DPYTHON_EXECUTABLE=' + pinterp + r' ' +
                r'-DPYTHON_INCLUDE_DIR=' + pypath + r'\\include' + r' ' +
                r'-DPYTHON_LIBRARY=' + pylib + r' ' +
                r'-DBoost_SERIALIZATION_LIBRARY_RELEASE=C:\\local\\lib\\libboost_serialization-mgw62-mt-1_63.dll'
                , verbose=True)
    run_command(r'mingw32-make install VERBOSE=1 -j2')
    ...
    cmake_opts = cmake_opts + r' ' + \
        ...
        r'-DBoost_CHRONO_LIBRARY_RELEASE=C:\\local\\lib\\libboost_chrono-mgw62-mt-1_63.dll' + r' ' + \
        r'-DCMAKE_CXX_FLAGS="-Wa,-mbig-obj"'
...
if is_python_build:
    ...
    # We must rename the file to core.pyd as wheel_setup defines that as a name. Only for python 3 pybind defines a different name.
    if  not python_version=='27':
