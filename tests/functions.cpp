...
{
    {
        ...
        BOOST_CHECK(EPSILON_COMPARE(pow(p1, gdual_d(3)), p1*p1*p1, 1e-12) == true);        // calls pow(gdual_d, gdual_d) with an integer exponent
        BOOST_CHECK(EPSILON_COMPARE(pow(p1, gdual_d(3.1)), pow(p1,3.1), 1e-12) == true);   // calls pow(gdual_d, gdual_d) with an real exponent
    }
    ...
    BOOST_CHECK(EPSILON_COMPARE(pow(p1 + 3.5, gdual_d(-1.1)), pow(p1 + 3.5, -1.1), 1e-12) == true);       // negative exponent (gdual_d, gdual_d)
    }
