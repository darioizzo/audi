...
BOOST_PYTHON_MODULE(core)
{
    ...
    bp::def(
        "sin_and_cos", +[](const gdual_v &d) { return pyaudi::v_to_l(sin_and_cos(d)); },
    ...
    bp::def(
        "sinh_and_cosh", +[](const gdual_v &d) { return pyaudi::v_to_l(sinh_and_cosh(d)); },
...
#if defined(AUDI_WITH_QUADMATH)
    // We expose the mppp::real128 class. NOTE: using boost::shared_ptr as holder is necessary for proper alignement of
    // __float128 by boost. Otherwise: segfault. See
    // http://webcache.googleusercontent.com/search?q=cache:xhGhABO-W58J:fhtagn.net/prog/2015/04/16/quaternion_boost_python.html&num=1&client=firefox-b&hl=en&gl=de&strip=1&vwsrc=0
    bp::class_<mppp::real128, boost::shared_ptr<mppp::real128>>(
        "real128", "Multiple precision float (a wrapper around __float128)")
        ...
        .def(bp::self != bp::self)
        .def(
            "__pow__", +[](const mppp::real128 &gd, double x) { return audi::pow(gd, x); },
            "Exponentiation (mppp::real128, double).")
        .def(
            "__pow__", +[](const mppp::real128 &base, const mppp::real128 &gd) { return audi::pow(base, gd); },
            "Exponentiation (mppp::real128, mppp::real128).")
        .def(
            "__rpow__", +[](const mppp::real128 &gd, double x) { return audi::pow(x, gd); },
    ...
    bp::def(
        "sin_and_cos", +[](const gdual_mp &d) { return pyaudi::v_to_l(sin_and_cos(d)); },
    ...
    bp::def(
        "sinh_and_cosh", +[](const gdual_mp &d) { return pyaudi::v_to_l(sinh_and_cosh(d)); },
